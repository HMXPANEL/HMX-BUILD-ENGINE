...
class ToolRunnerImplTest {
    ...
    fun `run passes working directory to process`() {
        ...
        every { processRunner.runWithTimeout(any(), any(), any(), match<ProcessConfig> { it.workingDir == workingDir }) } returns ProcessResult(0, "", "", 10)
    ...
    fun `run passes environment variables to process`() {
        ...
        every { sdkManager.findTool(any()) } returns Path.of("/usr/bin/javac")
        every { processRunner.runWithTimeout(any(), any(), any(), match<ProcessConfig> { it.environment["MY_VAR"] == "hello" }) } returns ProcessResult(0, "", "", 10)
